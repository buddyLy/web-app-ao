(function () {
    ...
    function Loyalty($timeout, $window, $location, config, logger, AssortmentoptimizationService, $routeParams, $scope) {
        ...
        function initialize() {
            ...
            vm.useStoreClustering = useStoreClustering;
        }
        ...
        function downloadItemMetrics() {
            var fileName = 'LoyaltyMetrics.csv';
            var fileType = 402;
		...
		function useStoreClustering(){
			if (vm.loyaltyRequest.useClustering){
				//if clustering checkbox is enabled, then second file is needed
				if ($scope.isFileChosen2){
